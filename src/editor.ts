...
import { SelectAllOp, SelectNoneOp, SelectInvertOp, SelectOp, HideSelectionOp, UnhideAllOp, DeleteSelectionOp, ResetOp, MultiOp, AddSplatOp, EntityTransformOp, type EditOp } from './edit-ops';
...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    const uniqueCopyName = (splat: Splat) => {
        const base = `${removeExtension(splat.name || splat.filename)} Copy`;
        const existing = new Set(
            (scene.getElementsByType(ElementType.splat) as Splat[]).map(s => s.name)
        );

        let result = base;
        let copyIndex = 2;
        while (existing.has(result)) {
            result = `${base} ${copyIndex++}`;
        }
    ...
    [
        ...
        'camera.viewportQuality', 'selection.changed', 'tool.coordSpace'
    ].forEach((eventName) => {
        events.on(eventName, () => {
    ...
    // camera viewport quality

    const setViewportQuality = (quality: number) => {
        const value = Math.max(0.5, Math.min(2, quality));
        if (value !== scene.config.camera.viewportQuality) {
            scene.config.camera.viewportQuality = value;
            events.fire('camera.viewportQuality', value);
        }
    };

    events.function('camera.viewportQuality', () => {
        return scene.config.camera.viewportQuality;
    });

    events.on('camera.setViewportQuality', (quality: number) => {
            ...
            if (mode === 'centers') {
                ...
                for (let y = 0; y < ph; ++y) {
                    for (let x = 0; x < pw; ++x) {
                        ...
                        if (mask.data[(my * mask.width + mx) * 4 + 3] === 255) {
    ...
    const performSelectionFunc = async (func: 'duplicate' | 'separate', target?: CloneTarget) => {
        ...
        if (data) {
            ...
            copy.name = uniqueCopyName(splat);
            ...
            if (func === 'separate') {
                ...
                await editHistory.add(ops.length > 1 ? new MultiOp(ops) : ops[0]);
            }
