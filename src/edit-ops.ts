...
class DeleteMaskOp extends StateOp {
    name = 'deleteMask';

    constructor(splat: Splat, mask: Uint8Array) {
        const state = splat.splatData.getProp('state') as Uint8Array;
        super(splat, IndexRanges.fromPredicate(splat.splatData.numSplats, (i) => {
            return mask[i] === 255 &&
                (state[i] & State.selected) !== 0 &&
                (state[i] & (State.locked | State.deleted)) === 0;
        }), State.deleted, BitOp.SET, State.deleted);
    }
}
