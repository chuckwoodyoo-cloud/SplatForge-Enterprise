...
class ExportPopup extends Container {
    ...
    constructor(events: Events, args = {}) {
        ...
        const spzVersionLabel = new Label({
            class: 'label',
            text: localize('popup.export.spz-version')
        });
        ...
        let currentExportType: ExportType = 'ply';
        let currentSplatNames: string[] = [];

        const getExportExtension = () => {
            switch (currentExportType) {
                case 'ply':
                    return compressBoolean.value ? '.compressed.ply' : '.ply';
                case 'splat':
                    return '.splat';
                case 'spz':
                    return '.spz';
                case 'sog':
                    return '.sog';
                case 'viewer':
                    return viewerTypeSelect.value === 'html' ? '.html' : '.zip';
            }
        };

        const getSplatIdx = (): 'all' | number => {
            if (splatSelect.value === 'all') {
                return 'all';
            }

            const idx = parseInt(splatSelect.value, 10);
            return Number.isFinite(idx) ? idx : 'all';
        };

        const getSelectedSplatName = () => {
            const splatIdx = getSplatIdx();
            return splatIdx === 'all' ? currentSplatNames[0] : currentSplatNames[splatIdx];
        };

        const updateFilenameFromSplat = () => {
            const name = getSelectedSplatName();
            if (name) {
                filenameEntry.value = name;
                updateExtension(getExportExtension());
            }
        };
        ...
        splatSelect.on('change', () => {
            updateFilenameFromSplat();
        });
        ...
        const reset = (exportType: ExportType, splatNames: string[], hasPoses: boolean) => {
            ...
            currentSplatNames = splatNames;

            const allRows = [
                viewerTypeRow, splatRow, animationRow, loopRow, colorRow, fovRow, compressRow, bandsRow, iterationsRow, spzVersionRow, filenameRow
            ];

            const activeRows = {
                ply: [splatRow, compressRow, bandsRow, filenameRow],
                splat: [splatRow, filenameRow],
                spz: [splatRow, bandsRow, spzVersionRow, filenameRow],
                sog: [splatRow, bandsRow, iterationsRow, filenameRow],
                viewer: [viewerTypeRow, splatRow, animationRow, loopRow, colorRow, fovRow, bandsRow, filenameRow]
            }[exportType];
            ...
            splatRow.hidden = splatNames.length < 2;
            splatSelect.options = [
                { v: 'all', t: localize('popup.export.splats-select.all') },
                ...splatNames.map((name, i) => ({ v: i.toString(), t: name }))
            ];
            splatSelect.value = 'all';
        ...
        this.show = (exportType: ExportType, splatNames: string[], showFilenameEdit: boolean) => {
            ...
            const assembleSpzOptions = () : SceneExportOptions => {
                return {
                    ...
                    spzVersion: parseInt(spzVersionSelect.value, 10) as 3 | 4
                };
            };
