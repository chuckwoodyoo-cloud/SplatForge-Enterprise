...
class ViewPanel extends Container {
    constructor(events: Events, tooltips: Tooltips, args = {}) {
        ...
        const viewportQualityRow = new Container({
            class: 'view-panel-row'
        });

        const viewportQualityLabel = new Label({
            text: localize('panel.view-options.viewport-quality'),
            class: 'view-panel-row-label'
        });

        const viewportQualitySelect = new SelectInput({
            class: 'view-panel-row-select',
            defaultValue: '1',
            options: [
                { v: '1', t: '1x' },
        ...
        viewportQualityRow.append(viewportQualityLabel);
        viewportQualityRow.append(viewportQualitySelect);
        ...
        this.append(fovRow);
        this.append(viewportQualityRow);
        ...
        events.on('camera.viewportQuality', (value: number) => {
            viewportQualitySelect.value = `${value}`;
        });

        viewportQualitySelect.on('change', (value: string) => {
            events.fire('camera.setViewportQuality', parseFloat(value));
        });
