...
class Menu extends Container {
    constructor(events: Events, args = {}) {
        ...
        const exportMenuPanel = new MenuPanel([{
            ...
            text: localize('menu.file.export.spz'),
            icon: createSvg(sceneExport),
            isEnabled: () => !events.invoke('scene.empty'),
        ...
        const selectionMenuPanel = new MenuPanel([{
            ...
            text: localize('menu.select.end-refinement'),
            icon: createSvg(selectDelete),
            extra: shortcutManager.formatShortcut('tool.endRefinement'),
            isEnabled: () => events.invoke('selection.splats'),
            onSelect: () => events.fire('tool.endRefinement')
        }, {
            ...
            text: localize('menu.select.clone-to-target'),
            icon: createSvg(selectDuplicate),
            extra: shortcutManager.formatShortcut('tool.cloneSelection'),
            isEnabled: () => events.invoke('selection.splats'),
            onSelect: () => events.fire('tool.cloneSelection')
        }, {
        ...
        const renderMenuPanel = new MenuPanel([{
            ...
            text: localize('menu.render.record-start'),
            icon: createSvg(sceneExport),
            isVisible: () => events.invoke('record.video.active') !== true,
            isEnabled: () => !events.invoke('scene.empty') && events.invoke('record.video.supported') !== false,
            onSelect: async () => await events.invoke('record.video.start')
        }, {
            text: localize('menu.render.record-stop'),
            icon: createSvg(sceneExport),
            isVisible: () => events.invoke('record.video.active') === true,
            onSelect: async () => await events.invoke('record.video.stop')
        }]);
