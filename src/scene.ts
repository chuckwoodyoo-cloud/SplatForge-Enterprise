...
class Scene {
    ...
    private onPreRender() {
        ...
        const quality = Math.max(0.25, this.config.camera.viewportQuality ?? 1);
        const requestedWidth = Math.ceil(device.width * quality / this.config.camera.pixelScale);
        const requestedHeight = Math.ceil(device.height * quality / this.config.camera.pixelScale);
        const maxTextureSize = (device as any).maxTextureSize ?? 8192;
        const limitScale = Math.max(1, requestedWidth / maxTextureSize, requestedHeight / maxTextureSize);
        this.targetSize.width = Math.ceil(requestedWidth / limitScale);
        this.targetSize.height = Math.ceil(requestedHeight / limitScale);
