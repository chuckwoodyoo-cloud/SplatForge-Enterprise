...
const fragmentShader = /* glsl*/ `
    ...
    void main(void) {
        ...
        // intersect ray with the active world plane. The plane offset lets the
        // editor grid follow the imported scene floor instead of staying fixed
        // at the world origin.
        vec4 activePlane = planes[plane];
        float planeOffset = plane == 0 ? plane_offsets.x : (plane == 1 ? plane_offsets.y : plane_offsets.z);
        activePlane.w = -planeOffset;

        float t;
        if (!intersectPlane(t, p, v, activePlane)) {
            discard;
        }
