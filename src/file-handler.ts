...
type ExportType = 'ply' | 'splat' | 'spz' | 'sog' | 'viewer';

type FileType = 'ply' | 'compressedPly' | 'splat' | 'spz' | 'sog' | 'htmlViewer' | 'packageViewer';
...
const initFileHandler = (scene: Scene, events: Events, dropTarget: HTMLElement) => {
    ...
    events.function('scene.write', async (fileType: FileType, options: SceneExportOptions, stream?: FileSystemWritableFileStream) => {
        // splat-transform exports have their own progress UI, other formats use spinner
